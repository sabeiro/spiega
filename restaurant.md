...
![capture rate](f_food/capture_rate.svg "capture rate")
_deriving capture rate from footfall und activities_

The underlying *model* can be described as

$$ g_{receipts}(t) f_{people/receipt}(t) = c_{rate}^a(t) a_{act}(t) f_{ext} + c_{rate}^f(t) a_{foot}(t) f_{ext} + b_{bias}(t) $$
...
- [x] we create a cilac specific mapping per location
...
![spatial degeneracy](f_food/spatial_degeneracy.png "spatial degeneracy")
...
![blue de](f_food/blue_de.png "blue de")
...
![dens interp](f_food/dens_interp.png "dens interp")
...
![isochrone](f_food/isochrone.png "isochrone")
...
![tile footfall](f_food/tile_footfall.png "footfall over a tile")
...
![visit hourly](f_food/visit_hourly.png "Hourly visit seris")
...
![visit daily](f_food/visit_daily.png "Daily visits")
_daily visits and footfalls over all locations_

Customer data are more regular than footfall data

![visit daily values](f_food/visit_series.png "time series of visits")
...
![matrix representation](f_food/input_data.png "input data in matrix representation")
_matrix representation of footfall and visits and their mutual correlation_

## Cilac mapping

We calculate daily values of activities per cilac and we use correlation as scoring to understand how the influence of neighboring cilacs can describe visitors.

We first calculate cilac cross correlation and keep only consistent cilacs

![cilac crosscor](f_food/cilac_crosscor.png "cilac cross corelation")
_cilac cross correlation_

We than run a regression to estimate the weighting of the cilac activities versus visits. We filter out the most significant weight and we rerun the regression.

We do a consistency check and cluster the cells as restaurant activities.

If we consider all cilacs with weight 1 we score 11% (of the location over 0.6 correlation)
![map unweighted](f_food/cilacMap_weighting.png "map weighted")
_unweighted and weighted cilac mapping_

If we weight all cilacs performing a linear regression we score 66%
...
![boxplot foot_vist](f_food/boxplot_foot_vist.png "boxplot foot_vist")
_boxplot foot_vist_

If we consider daily values activities and footfall don't correlate at first with reference data.

![kpi cross](f_food/kpi_cross.png "kpi cross")
...
* maps the cilacs giving a specific weight (bayesian regressor)
* groups the cilacs and sum up the activities for the specific location - performance 10%
...
![kpi hybrid](f_food/kpi_hybrid.png "kpi hybrid")
...
![feature correlation](f_food/feature_correlation.png "correlation of features")
_correlation between features_

We select the features which have the higher variance.
![feature_variance](f_food/feature_variance.png "variance of features")
_relative variance of single features_

We can see that the selected variances are mutally well distributed
![feature selection](f_food/feature_selection.jpg "mutual distribution of selected features")
...
![feature knock out](f_food/feature_knockout_fix.png "feature knock out")
...
![feature knock out](f_food/feature_knockout.png "feature knock out")
...
![ref_vs_pred](f_food/ref_vs_pred.png "reference versus prediction")
...
![time series](f_food/time_series.png "time series")
...
![longShort_hist](f_food/longShort_hist.png "long short histogram")
_learning cycle_
...
![score history](f_food/score_history.png "score history")
_score history_
...
![kpi_hist](f_food/kpi_hist.png "kpi histogram")
_kpi histograms_
...
![kpi_boxplot](f_food/kpi_boxplot.png "kpi boxplot")
