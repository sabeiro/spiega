...
category: #tech
roam_refs: programming practices
roam_aliases: ["programming practices"]
output: 
	md_document:
...
This document is intended to list all the programming practices improved after a couple of decades of experience and intended to coordinate agentic development. 
The main features are:

* reliability: each project needs a separate folder called `/test` where scripts run tests useful for development and production
* **ease**: the code should be minimal and easy to browse and read. Classes should be specific and abstraction is discouraged. Every repository needs to have an `/example` folder where data is loaded and functions executed so it's expected to know how the software would behave with real data and tell 
* **configuration**: every project should have a master json which is used to store all the options across all stages: (environment, preprocessing, loading, input, output, stages...). The same conf file will have a 'run' key which will log all the important activities during the run (training, KPIs, execution time...). That conf is the unique source of truth for a specific configuration and no other parameter should be defined elsewhere (apart from confidential keys). The same conf will be passed within function and the software should make sure keys are not overwritten if not required.
* **functions**: similar to configuration each file should have a set list of utilities and libraries within the scope of the file. Functions should be organized per scope and dependencies avoiding to create interdependencies between libraries. The main application should be able to call the different modules without moving too much between libraries. The main functions should initiate every variable or class avoiding the modules to call each other.
* **definitions**: no global definition apart what is inside the conf file and environment variables. Particular attention for async and callback functions
