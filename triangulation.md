...
![quality](f_triang/quality.png "quality")
...
![precise_bse](f_triang/precise_bse.png "precise bse")
...
![neighboring_base](f_triang/neighboring_bse.png "neighboring cells")
...
![cell_geometry](f_triang/cell_geometry.png "cell geometry")
_we map the location into a geometry (mtc, zip5)_

But for some technology we have centroids outside the coverage area

![cell_no_coverage](f_triang/centroid_no_coverage.png "cell no coverage")
...
![stability_displacement](f_triang/stability_displacement.png "stability displacement")
_stability of counts after displacement_

We than check the stability of counts by area 

![stability_area](f_triang/stability_area.png "stability area")
...
![poly_cilac](f_triang/poly_cilac.png "f_triang/poly_cilac")
...
![dens_cilac](f_triang/dens_cilac.png "f_triang/dens_cilac")

We can divide the space where each cell centroid sits at the vertex of a Delaunay triangle.
![poly_delaunay](f_triang/poly_delaunay.png "f_triang/poly_delaunay")
_Delaunay triangulation on cell centroids_

### Triangulation

If we average over all vertex positions we have a more sparse activity distribution. We can even define a standard error for each activity [filter_activities.py](http://172.25.186.11:8000/gmarelli/geomadi/blob/master/proc/filter_activities.py). 
...
![activity_triang](f_triang/activity_triang.png "f_triang/activity_triang")
...
![density_trian](f_triang/density_triang.png "f_triang/density_trian")
...
![error_dist](f_triang/error_dist.png "f_triang/error_dist")
...
![poly_buffer](f_triang/poly_buffer.png "f_triang/poly_buffer")
...
![octree](f_triang/octree.svg "octree 2d")
_2d representation of an octree_

![octree](f_triang/octree.png "octree 3d")
_3d usage of an octree_


To anonymize the data we loop until we find the minimum number of digits for each geohash string: [fenics_finite.py](http://172.25.186.11:8000/gmarelli/geomadi/blob/master/py/fenics_finite.py#L155). 
```python
...
![geohash binning](f_triang/geohash_binning.png "geohash binning")
...
![geohash buffer](f_triang/geohash_buffer.png "geohash buffer")
_Overcome the anonimization problem using geohash binning_

Alternatively we can apply a double binary octree
![octree binning](f_triang/octree_binning.png "octree binning")
...
![mapping generation](f_triang/mapping_generation.svg "mapping generations")
...
The spatial resolution is given by the distribution of cells which modifies the definition of space around. We represent each cell centroid as a trapezoid whose center corresponds to the cell centroid and the area to a portion of the BSE area [fenics_finite.py](http://172.25.186.11:8000/gmarelli/geomadi/blob/master/geomadi/geo_octree.py). 
...
![addyn_sol](f_triang/addyn_sol.png "f_triang/addyn_sol")
...
![addyn_surf](f_triang/addyn_surf.png "f_triang/addyn_surf")
...
![space gradient](f_triang/space_gradient.png "gradient of the space deformation")
...
Finally we update the mapping  [etl_roda.py](http://172.25.186.11:8000/gmarelli/geomadi/blob/master/etl/etl_roda.py). 
